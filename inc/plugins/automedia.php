...
// Plugin version
define('AUTOMEDIA_VER', '4101');
define('AUTOMEDIA_NR', '4.1.1');
...
function automedia_info()
{
    ...
    $am_info = array(
        ...
        "website"       => "http://doylecc.altervista.org",
        "author"        => "doylecc",
        "authorsite"    => "http://doylecc.altervista.org",
        "version"       => "4.1.1",
        "compatibility"     => "18*",
        "codename"      => "automedia"
        );
...
function automedia_more_info()
{
    ...
    if (!isset($lang->av_not_upgraded)) {
        $lang->load("automedia");
    }

    // Display upgrade message
    $cached_version = $cache->read('automedia');
    if (!is_array($cached_version)
        || (is_array($cached_version)
            && (!isset($cached_version['release'])
            || $cached_version['release'] != AUTOMEDIA_NR)
        )
    ) {
        $status = '<div style="font-size:1.2em; margin-top:7px; text-align:center;';
        $status .= ' color:red;">';
        $status .= $lang->av_not_upgraded.'</div>';
    } else {
